...
Callback_PlayerConnect()
{
    ...
    self thread namechecker::nameChecker();
...
Callback_PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc)
{
	...
	if(!isdefined(reflect))
	{
		...
		if(iDamage < 1)
		    ...
		    self oneshot::_finishPlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc);
	}
	else
	{
		...
		eAttacker oneshot::_finishPlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc);
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc)
{
	...
	if(doKillcam && !level.roundended)
		self thread killcam::killcam(attackerNum, delay);
	else
	{
...
*/
    ...
    if((getCvar("scr_roundcam") == "1") && (!timeexpired) && (game["matchstarted"]))
    {
            ...
            || level.gametype == "re" && ((isdefined(level.playercam) || isdefined(level.goalcam)) && roundwinner != "draw" && roundwinner != "reset"))
        {
            ...
            if (players.size > 0)
            {
                utils::savePlayerWeapons();
                level thread killcam::roundcam_level(timeWaitedBeforeRoundcam, roundwinner, videoDurationBeforeKill);
                for (i = 0; i < players.size; i++)
                {
                    players[i] thread killcam::roundcam_client(timeWaitedBeforeRoundcam, videoDurationBeforeKill);
                }
                level waittill("roundcam_ended");
            }
            else
...
bomb_countdown()
{
	...
	countdowntime = 60;
//    thread codsu::bomb_cd(countdowntime);
